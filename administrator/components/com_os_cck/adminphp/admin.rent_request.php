...
class AdminRent_request{
  static function showRentRequestInstances($option){
    global $db, $app, $jConf;
    ...
    //           ."\n LEFT JOIN #__os_cck_layout as lay ON lay.fk_eid = ent.eid WHERE lay.type = 'rent_request_instance'";
      ...
      foreach ($rows as $row) {
        ...
        foreach($extra_fields_list as $Fieldvalue){
          if($Fieldvalue->show_in_instance_menu && strpos($layout_html,"{|f-".$Fieldvalue->fid."|}")){
            $fieldNames[$key]['ent_name'] = $entity->name;
    ...
    AdminViewRent_request :: showRentRequestInstances($option, $rows, $clist, $publist, $search, $pageNav, $sort_arr, $show_fields,$entity_list);
  }

  static function editRentRequestInstance($option, $bid){
    global $db, $user, $app;
  ...
  static function remove_rent_request_item($bid, $option){
    global $db,$app;
    ...
    $app->redirect("index.php?option=$option&task=show_rent_request_instances");
  }
  ...
  static function accept_rent_requests($option, $bids){
    ...
    global $db,$app;
    foreach ($bids as $bid) {
      $query = "SELECT fid_parent FROM #__os_cck_child_parent_connect WHERE fid_child=$bid";
      $db->setQuery($query);
      $parent_id = $db->loadResult();

      $rent_request = new os_cckEntityInstance($db);
      $rent_request->load($parent_id);
      $rent_request->child_id = $bid;
    ...
    $app->redirect("index.php?option=$option&task=show_rent_request_instances",'ok');
  }
    ...
    foreach ($bids as $bid) {
      ...
      $buying_request = new os_cckEntityInstance($db);
      $buying_request->load($parent_id);
      $buying_request->child_id = $bid;


      $entity = new os_cckEntity($db);
      $entity->load($buying_request->fk_eid);
      $buying_request->entity_name = $entity->name;
    ...
    $app->redirect("index.php?option=$option&task=show_buy_request_instances",'ok');
  }
