...
<!-- layout notice -->
  <?php 
    noticeLinkToDocLayout('show_review_instance', 'Show Review Layout', 'http://ordasoft.com/News/OS-CCK-Documentation/cck-term-explanation.html#Show / Add Review Layout');
  ?>
<!-- /layout notice -->
            ...
            <div class="main-fields-content">
              ...
              for ($i = 0; $i < count($fields); $i++) {
                ...
                if($field->published == "1" && $field->field_type != "captcha_field"){
                  echo show_edite_add_form_field_layout($field, 'show',$fields_from_params,$layout->type);
                }
              }
              if(count($layout_params['unique_fields'])){
                foreach ($layout_params['unique_fields'] as $key => $custom_options){
                  require getLayoutPathCCK::getAdminLayoutViewPath('com_os_cck', 'show_review_instance', 'customField');
                }
              } ?>
            </div>
        ...
        <div id="options-tab-3">
          <div id="form-options-accordion">
            ...
            <div class="main-fields-content">
              ...
              <div class="category-options">
                <label><?php echo JText::_('COM_OS_CCK_LAYOUT_SHOW_NO_REVIEW_MESSAGE'); ?></label>
                <?php echo $layout->no_review_message; ?>
              </div>
            </div>
            <h3><?php echo JText::_("COM_OS_CCK_STYLING_LABEL_ACCORDION_FIELDS_CSS_OPTIONS") ?></h3>
            <div class="styling-field-content">
