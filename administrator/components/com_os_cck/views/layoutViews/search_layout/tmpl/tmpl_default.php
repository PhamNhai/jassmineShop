...
$cck_send_button->label = JText::_("COM_OS_CCK_SEARCH_BUTTON");
$cck_send_button->db_field_name = 'cck_send_button';
$cck_send_button->description = '';

$cck_search_field = new stdClass();
$cck_search_field->label = JText::_("COM_OS_CCK_SHOW_SEARCH");
$cck_search_field->db_field_name = 'cck_search_field';
$cck_search_field->description = '';

$unique_fields = array($cck_send_button , $cck_search_field);
...
<!-- layout notice -->
  <?php 
    noticeLinkToDocLayout('search_layout', 'Search Layout', 'http://ordasoft.com/News/OS-CCK-Documentation/cck-term-explanation.html#Search Layout');
  ?>
<!-- /layout notice -->
...
<?php 
      ...
      $fields = array();
      foreach ($layout->fields_list as $field) {
        $ftype = $field->field_type;
        if ($ftype == 'decimal_textfield'
            || $ftype == 'datetime_popup'
            || $ftype == 'text_radio_buttons'
            || $ftype == 'text_textfield'
            || $ftype == 'categoryfield'
            || $ftype == 'text_textarea'
            || $ftype == 'text_url'
            || $ftype == 'text_single_checkbox_onoff'
        ...
        <div id="options-tab-1">
          <div id="fields-options-accordion">
            ...
            <div class="main-fields-content">
              <?php
              foreach ($unique_fields as $field) {
                require getLayoutPathCCK::getAdminLayoutViewPath('com_os_cck', 'search_layout', 'fieldUniq');
              }
              foreach ($layout->fields_list as $field) {
                $ftype = $field->field_type;
                if($field->published != "1")continue;
                if ($ftype == 'decimal_textfield'
                    || $ftype == 'datetime_popup'
                    || $ftype == 'text_radio_buttons'
                    || $ftype == 'text_textfield'
                    ...
                    || $ftype == 'text_single_checkbox_onoff'
                    || $ftype == 'text_select_list'){
                    echo show_edite_add_form_field_layout($field, 'search',$fields_from_params,$layout->type);
                }
              }
              if(count($layout_params['unique_fields'])){
                foreach ($layout_params['unique_fields'] as $key => $custom_options){
                  require getLayoutPathCCK::getAdminLayoutViewPath('com_os_cck', 'search_layout', 'customField');
                }
              }
              ?>
            </div>
        ...
        <div id="options-tab-3">
          ...
          <h3><?php echo JText::_("COM_OS_CCK_STYLING_LABEL_ACCORDION_FIELDS_MAIN_OPTIONS") ?></h3>
            <div class="main-fields-content">
              ...
              <div class="category-options">
                <label><?php echo JText::_("COM_OS_CCK_SEARCH_RESULT_DISPLAY_LAYOUT"); ?></label>
