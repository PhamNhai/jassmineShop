...
<!-- layout notice -->
  <?php 
    noticeLinkToDocLayout('all_categories_layout', 'All categories layout', 'http://ordasoft.com/News/OS-CCK-Documentation/cck-term-explanation.html#Show all categories');
  ?>
<!-- /layout notice -->
            ...
            <div class="main-fields-content">
              ...
              if(isset($layout_params['views']['show_request_layout'])){
                foreach ($layout_params['views']['show_request_layout'] as $key => $value) {
                  require getLayoutPathCCK::getAdminLayoutViewPath('com_os_cck', 'all_categories_layout', 'layoutOptions');
                }
              }
              if(count($layout_params['unique_fields'])){
                foreach ($layout_params['unique_fields'] as $key => $custom_options){
                  require getLayoutPathCCK::getAdminLayoutViewPath('com_os_cck', 'all_categories_layout', 'customField');
                }
              } ?>
            </div>
        ...
        <div id="options-tab-3">
          ...
          <h3><?php echo JText::_("COM_OS_CCK_STYLING_LABEL_ACCORDION_FIELDS_MAIN_OPTIONS") ?></h3>
            <div class="main-fields-content">
              ...
              <div class="category-options">
                <label><?php echo JText::_('COM_OS_CCK_HEADER_CATEGORY_LAYOUT'); ?> <i title="<?php echo JText::_("COM_OS_CCK_HEADER_CATEGORY_LAYOUT_TOOLTIP")?>" class="glyphicon glyphicon-info-sign date_tooltip"></i></label>
                <?php echo $layout->categoryLayout; ?>
              </div>
            </div>
