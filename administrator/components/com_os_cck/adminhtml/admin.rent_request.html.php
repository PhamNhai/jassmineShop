...
class AdminViewRent_request{
  ...
  static function editRentRequestInstance($option, & $entityInstance, & $str_list){


    global $os_cck_configuration,$my, $app, $session;
    $html = "<div class='os_cck_caption' ><img src='./components/com_os_cck/images/os_cck_logo.png' alt ='Config' />" . JText::_('COM_OS_CCK_ADMIN_REQUEST_RENT') . "</div>";
    $app = JFactory::getApplication();
    $app->JComponentTitle = $html;
    ...
    $doc->addScript('//maps.google.com/maps/api/js');
  ...
  $html .='</'.$shell_tag.'>';
    ...
    ?><hr><?php
    if(!empty($str_list['parent_instance'])){
        AdminRent_request::editRentRequestInstance($option, $str_list['parent_instance']);
        ?>
