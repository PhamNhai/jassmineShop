...
<div class="cck-body">
  ...
  if($show_type == 2){?>
    <input <?php echo $button_style['field_styling'];?>  type="button" class="button btn btn-info <?php echo $button_style['custom_class'];?>" value="<?php echo ($layout_params['button_name'])?$layout_params['button_name'] : JText::_('COM_OS_CCK_BUTTON_SHOW_FORM_SAVE'); ?>"
  ...
  <form class="<?php echo $form_custom_class; ?>" <?php echo $form_styling; ?> action="index.php" method="post" name="<?php echo $unique_form_id?>" id="<?php echo $unique_form_id?>" enctype="multipart/form-data">
    ...
    ?>
      <input <?php echo $field_styling; ?> type="button" name="save_button" value="<?php echo $buttonText; ?>" class="button <?php echo $custom_class; ?>" onclick="javascript:sendRequest<?php echo $layout->type?>('save_instance', <?php echo $unique_form_id?>);">
    <?php
