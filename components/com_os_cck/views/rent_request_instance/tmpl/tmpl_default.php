...
<div class="cck-body">
  ...
  if($show_type == 3){
    ...
    ?>
    <a <?php echo $button_style['field_styling'];?> class="button <?php echo $button_style['custom_class'];?>" href="<?php echo $link ?>">
      <?php echo ($layout_params['button_name'])?$layout_params['button_name'] : JText::_('COM_OS_CCK_BUTTON_SHOW_FORM_RENT_REQUEST'); ?>
    </a>
    <?php
    echo "</div>"; //close <div class="cck-body">
    return;
  ...
  foreach ($fields_list as $value) {
    if($value->field_type == 'datetime_popup'){
      if($field_from_params[$value->db_field_name.'_field_type'] == 'rent_from'){
        $field_from_capt = $value->db_field_name;
        $from = true;
      }
      if($field_from_params[$value->db_field_name.'_field_type'] == 'rent_to'){
        $field_to_capt = $value->db_field_name;
        $to = true;
      }
    }
  }
  ...
  if(!($from && $to)){
    JError::raiseWarning(0, JText::_("COM_OS_CCK_ERROR_NO_RENT_CALENDAR") );
    echo "</div>"; //close <div class="cck-body">
    return;
  }

  if($show_type == 2){?>
    <input <?php echo $button_style['field_styling'];?> type="button" class="button btn btn-info <?php echo $button_style['custom_class'];?>" value="<?php echo ($layout_params['button_name'])?$layout_params['button_name'] : JText::_('COM_OS_CCK_BUTTON_SHOW_FORM_RENT_REQUEST'); ?>"
  ...
  <form class="<?php echo $form_custom_class; ?>" <?php echo $form_styling; ?> action="index.php" method="post" name="<?php echo $unique_form_id?>" id="<?php echo $unique_form_id?>" enctype="multipart/form-data">
    ...
    $buttonText = isset($layout_params["views"]["layout_button_text"])
                  && !empty($layout_params["views"]["layout_button_text"])?$layout_params["views"]["layout_button_text"]:JText::_('COM_OS_CCK_BUTTON_FORM_RENT_REQUEST');
    ?>
    <input <?php echo $field_styling; ?> type="button" name="rent_request_button" value="<?php echo $buttonText; ?>"
           class="button <?php echo $custom_class; ?>" onclick="javascript:sendRequest<?php echo $layout->type?>('send_rent_request',<?php echo $unique_form_id?>);">
    <?php    
    ...
    <input type="hidden" name="time_until" value="">
...
<script>
  ...
  //*****************   begin add for show/hiden button "Add review" ********************
    ...
    function sendRequest<?php echo $layout->type?>(task,form_name){
      ...
      //rent request fields must be whole filled
      if(task == 'send_rent_request'){

        if(jQuerCCK(form).find('[name=rent_from]').val() == ''){
          alert("Please, choose date from");
          return;
        }
        if(jQuerCCK(form).find('[name=rent_until]').val() == ''){
        ...
        if(<?php echo  $os_cck_configuration->get('by_time');?>){
          ...
          if(jQuerCCK(form).find('[name=time_from]').val() == ''){
            alert("Please, choose time from");
            return;
          }
          if(jQuerCCK(form).find('[name=time_until]').val() == ''){
    ...
    // dfrom = '<?php echo $field_from_capt;?>'
    // dto = '<?php echo $field_to_capt;?>'


    // jQuerCCK('input[name=fi_'+dfrom+'], input[name~=fi_'+dto+']').click(function()
    // {
        
           
    //         var type = '<?php echo $layout->type;?>'
